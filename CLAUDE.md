# CLAUDE.md — Native ramp-quote workspace (laulima26, cloud session)

Native-groups ramp demo on **laulima26**, running as the conversational demo from a Claude
Code on the web session. This repo root stands in for the `demos/native/` folder in
...
token, and writes `.mcp.json` at this repo's root automatically at session start. Everything
you need for the demo itself is pre-resolved below.
...
On this prompt, **always run the parallel build below** — it is the default path, not an option. Read
`MomentumCallNotes__c` for Acme, interpret the deal, then build both quotes the native way via two
concurrent subagents. No exploration, no existence checks, no catalog lookups — everything is
pre-resolved. All tools ride ONE multiplexed connector: `soqlQuery` routes to the cohort server, the
ramp tools to the native server.

### Two modes — FLAT (default) vs FAITHFUL

The plain prompt above builds **FLAT** (fast, level segments — see the shape caveat). To build the
...
re-verified from this cloud session on 2026-09-09 (script run, not conversational): 3-yr quote
...
## Build chain — the fixed MCP sequence each subagent runs
...
## Parallel build — two subagents, orchestrator narrates
...
## Presentation — in-chat table, always linked
...
## How to talk (a business conversation, not a tooling session)
...
the container, patches its `tools/ramp_auth.py` with a headless `client_credentials` grant
